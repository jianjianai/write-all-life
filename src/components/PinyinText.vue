...
let shwoArray: Ref<{ t: string, up?: string, is_?:boolean}[]> = ref([]);
//显示和隐藏文字
const xxxxxsssss = ()=>{
        ...
        for (let wordInex = 0; wordInex < word.value.length; wordInex++) {
            ...
            if (!shwoWord.value) {
                if(inputWord.value[wordInex]){
                    ...
                    shwoArray.value[shwoIndex].t = "";
                }
            } else {
                shwoArray.value[shwoIndex].t = word.value[wordInex];
            }
        }
        lastIndex = theIdex;
...
const wwwzzzpppyyy = ()=>{
    ...
    for (const theChar of text.value) {
        shwoArray.value.push({ t: theChar });
    }
...
//党显示状态变化时
watch([shwoWord,inputWord],()=>{xxxxxsssss()},{ immediate: false });
